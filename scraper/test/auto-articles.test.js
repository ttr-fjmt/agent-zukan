...
const sampleArticle = {
  ...
  sections: [
    { heading: '1', body: ['あ'.repeat(400)], quotes: [{ source_id: 'sample-source', text: '許可の有効期間は3年です。' }] },
    { heading: '2', body: ['い'.repeat(400)], quotes: [{ source_id: 'sample-source', text: '許可の有効期間は3年です。' }] },
    { heading: '3', body: ['う'.repeat(400)], quotes: [{ source_id: 'sample-source', text: '許可の有効期間は3年です。' }] },
    { heading: CLOSING_HEADING, body: ['え'.repeat(100)], quotes: [] },
  ],
};
const sampleRaw = () => 'ここに公式の説明があります。許可の有効期間は3年です。以上。';
...
test('出典リストは、すべて公式ドメインのページを指している', () => {
  ...
  for (const s of sources) {
    ...
    assert.match(
      s.url,
      /^https:\/\/([a-z0-9-]+\.)*(mhlw\.go\.jp|jesra\.or\.jp)\//,
      `${s.id} が公式のページではない`
    );
  }
...
test('引用を1文字でも言い換えると、検査に落ちる', () => {
  ...
  tampered.sections[0].quotes[0].text = '許可の有効期間は最長3年です。';
...
test('引用に無い数字を本文に書くと、検査に落ちる', () => {
  ...
  const tampered = JSON.parse(JSON.stringify(sampleArticle));
  tampered.sections[0].body[0] += '手数料は5年ごとに見直されます。';
...
test('割合（％）を書くと、検査に落ちる', () => {
  ...
  const tampered = JSON.parse(JSON.stringify(sampleArticle));
  tampered.sections[0].body[0] += '手数料は30%です。';
  assert.ok(checkNoPercent(tampered).length > 0, '料率を見逃している');
});
...
test('自動で書いた記事のページが、書き出されていて中身と一致している', () => {
  ...
  for (const guide of loadAutoGuides()) {
    ...
    assert.ok(
      html.includes(`<link rel="canonical" href="https://agent-zukan.net/guide/${guide.slug}/">`),
