 ...
 *   2. 本文に書いた数字（710円・令和6年4月1日など）が、同じ節の引用に実在するか
 *   3. 断定・誇大な表現（「必ず転職できます」など）が無いか
 *   4. 手数料の料率（％）を書いていないか（公式で確認できないため、書かない約束）
...
/**
 * 手数料の料率（％）を書いていないか。
 * 上限制手数料の料率は公式ページで確認できていないため、このサイトでは書かない
 * （既存の記事にも同じ線引きが効いている。README「解説記事」の節）。
 */
function checkNoPercent(article) {
  const text = textOf(article);
  return /\d+(\.\d+)?\s*[%％]/.test(text)
    ? ['割合（％）は書けません。料率を公式ページで確認できていないためです']
    : [];
}
